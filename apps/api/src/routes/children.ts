...
childRoutes.put('/children/:id', authorize('admin'), async (c) => {
  const { firstName, lastName, gender, guardianPhone, consentObtained, isActive } =
    await c.req.json<{ firstName?: string; lastName?: string; gender?: 'M' | 'F'; guardianPhone?: string; consentObtained?: boolean; isActive?: boolean }>()
