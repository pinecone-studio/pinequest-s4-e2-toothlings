...
const CarryForwardModal = ({ open, onClose, source, schoolName, submitting, onSubmit }: Props) => {
  ...
  const handleSubmit = async () => {
    ...
    if (wantReminder && scheduledAt) {
      downloadICS({
        title: `Шүдний скрининг — ${schoolName} · ${name || source.name}`,
        description: `${seasonId} улирлын скрининг айлчлал. Хамрагдах сурагч: ${source.enrolled}.`,
  ...
  return (
    <Modal
      ...
      footer={
        <>
          <button onClick={onClose} className="btn rounded-xl px-4 py-2 text-[13px] font-medium text-text-muted transition-all duration-150 hover:text-text-base">
            Болих
          </button>
          <button
