...
import { ClipboardDocumentListIcon, MagnifyingGlassIcon, TrashIcon } from '@heroicons/react/24/solid'
      ...
      <div className="flex flex-wrap items-center gap-2">
        ...
        {classes.length > 1 && (
          <>
            ...
            <button onClick={() => setClassFilter('')} className={`btn rounded-full px-3 py-1.5 text-[12px] font-semibold transition-all ${classFilter === '' ? 'bg-primary text-text-on-primary' : 'border border-border bg-surface text-text-muted hover:border-primary hover:text-primary'}`}>
              Бүх анги
            </button>
            {classes.map((c) => (
              <button key={c} onClick={() => setClassFilter(c === classFilter ? '' : c)} className={`btn rounded-full px-3 py-1.5 text-[12px] font-semibold transition-all ${classFilter === c ? 'bg-primary text-text-on-primary' : 'border border-border bg-surface text-text-muted hover:border-primary hover:text-primary'}`}>
                {c}
              </button>
            ))}
          </>
        ...
        <div className="h-5 w-px bg-border" />
        {LEVELS.map((l) => (
          <button key={l.value} onClick={() => setLevelFilter(l.value)} className={`btn rounded-full px-3 py-1.5 text-[12px] font-semibold transition-all ${levelFilter === l.value ? 'bg-primary text-text-on-primary' : 'border border-border bg-surface text-text-muted hover:border-primary hover:text-primary'}`}>
            {l.label}
          </button>
        ))}

        {(search || classFilter || levelFilter) && (
          <Button variant="ghost" size="sm" aria-label="Цэвэрлэх" onClick={() => { setSearch(''); setClassFilter(''); setLevelFilter('') }}><TrashIcon className="size-4" /></Button>
        )}
      </div>
